...
| PII/secrets leakage | Structured allowlisted logging, opaque identifiers, no raw answers/tokens/emails by default, secret scanning, synthetic fixtures. Assessment events may carry a SHA-256 seed fingerprint for correlation but never the seed or grading contract. | Log/event inspection and repository scans. |
...
Assessment seeds are cryptographic authority material, not client data. They are protected at rest and absent from API/outbox payloads; only a one-way SHA-256 fingerprint is retained for operational correlation. Correct-answer/grading snapshots remain server-private.
