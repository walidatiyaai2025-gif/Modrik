...
Do not commit or share the live password. After changing the Backend `.env`, run the PHP 8.4 CLI commands from the Backend root:
...
The Demo Admin seeder is active only while `MODRIK_FIXTURE_MODE=true`. If neither Admin environment value is supplied, it is a no-op. If only one is supplied, or the password is shorter than 12 characters, it fails closed.
