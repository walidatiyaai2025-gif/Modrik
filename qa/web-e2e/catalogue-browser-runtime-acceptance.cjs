...
async function learningViewport(browser, spec) {
  ...
  try {
    ...
    check(await nav.count() === 6, "E2E_LEARNING_NAV_COUNT");
    ...
    check(await nav.nth(1).getAttribute("aria-current") === "page", "E2E_CATALOGUE_DESTINATION");
...
async function stateAcceptance(browser) {
  ...
  try {
    ...
    await page.locator(".student-nav button").nth(5).click();
