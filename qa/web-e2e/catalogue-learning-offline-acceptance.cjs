...
const candidate = process.env.MODRIK_E2E_CANDIDATE || "current-tree-catalogue-learning-offline";
...
const evidence = {
  schema_version: "modrik.web.catalogue-learning-offline-browser-evidence.v1",
  candidate,
...
function envelope(data) {
  return { data, meta: { request_id: "catalogue-offline-e2e" } };
}
...
function problem(status, code, detail) {
  return {
    ...
    code,
    detail,
    request_id: "catalogue-offline-e2e",
...
function cataloguePayload() {
  return {
    ...
    context: {
      ...
      track_title: { en: "Grade 6 published curriculum", ar: "المنهج المنشور للصف السادس", fr: "Programme publié de 6e année" },
    },
    subjects: [{
      id: ids.subject,
...
async function handleMock(req, res) {
  ...
  if (pathname === "/v1/content-catalogue") return sendJson(res, 200, envelope(cataloguePayload()));
  ...
  if (pathname === "/v1/academic-tracks") {
    return sendJson(res, 200, envelope({ tracks: [{
      ...
      labels: {
        en: "Published curriculum track for offline browser evidence",
        ar: "مسار منهج منشور لاختبار وضع عدم الاتصال في المتصفح",
        fr: "Parcours de programme publié pour la preuve hors ligne du navigateur",
      },
    }] }));
  }

  return sendJson(res, 404, problem(404, "RESOURCE_NOT_FOUND", "Catalogue offline route not found."), "application/problem+json");
}
...
async function waitForHttp(url, timeoutMs = 30000) {
  ...
  fail("E2E_CATALOGUE_OFFLINE_APP_START_TIMEOUT");
}
...
function startNext() {
  ...
  return spawn(process.execPath, [nextBin, "start", "-H", "127.0.0.1", "-p", String(appPort)], {
    ...
    env: {
      ...
      MODRIK_BUILD_VERSION: "catalogue-offline-e2e",
...
async function main() {
  check(fs.existsSync(path.join(appDir, "package.json")), "E2E_CATALOGUE_OFFLINE_TARGET_MISSING");
  fs.mkdirSync(evidenceDir, { recursive: true });

  const mock = http.createServer((req, res) => {
    handleMock(req, res).catch(() => {
      if (!res.headersSent) sendJson(res, 500, { code: "E2E_CATALOGUE_OFFLINE_MOCK_FAILURE" });
      else res.end();
    });
  });
  ...
  try {
    ...
    await reachable(lessonButton, page, "E2E_CATALOGUE_OFFLINE_LESSON_ACTION");
    await lessonButton.click();
    await page.locator(".lesson-block").first().waitFor({ state: "visible", timeout: 10000 });

    await page.locator(".student-nav button").nth(0).click();
    ...
    check(!(await start.isDisabled()), "E2E_CATALOGUE_OFFLINE_ONLINE_START_DISABLED");
    ...
    const practice = page.locator(".student-nav button").nth(2);
    ...
    await page.locator(".student-shell").waitFor({ state: "visible", timeout: 15000 });
    await start.waitFor({ state: "visible", timeout: 10000 });
    check(!(await start.isDisabled()), "E2E_CATALOGUE_OFFLINE_RECOVERY_START_DISABLED");
    await noHorizontalOverflow(page, "E2E_CATALOGUE_OFFLINE_RECOVERY_HORIZONTAL_OVERFLOW");
    ...
    evidence.failure_code = /^E2E_[A-Z0-9_:-]+$/.test(message)
      ? message
      : "E2E_CATALOGUE_OFFLINE_ASSERTION_FAILED";
  } finally {
  ...
  if (evidence.status === "PASS") {
    console.log("Catalogue learning offline browser acceptance: PASS");
  } else {
    console.error(`Catalogue learning offline browser acceptance: FAIL (${evidence.failure_code})`);
    process.exitCode = 1;
  }
}
...
main().catch(() => {
  evidence.status = "FAIL";
  evidence.failure_code = "E2E_CATALOGUE_OFFLINE_HARNESS_FAILURE";
  fs.mkdirSync(evidenceDir, { recursive: true });
  fs.writeFileSync(
  ...
  console.error("Catalogue learning offline browser acceptance aborted: E2E_CATALOGUE_OFFLINE_HARNESS_FAILURE");
  process.exitCode = 1;
});
