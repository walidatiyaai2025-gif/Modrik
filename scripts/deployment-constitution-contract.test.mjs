...
test("LiteSpeed canonical startup is the direct Next standalone server", () => {
  assert.match(constitution, /Canonical startup: the generated Next standalone `server\.js`/);
  assert.match(constitution, /`startup\.cjs` may exist only as a compatibility\/rollback bridge/);
  assert.match(runbook, /server\.js\s+# canonical LiteSpeed startup/);
  assert.match(runbook, /Startup file: \*\*`<WEB_APPLICATION_ROOT>\/server\.js`\*\*/);
  assert.match(packager, /Standalone Next app release identity is missing/);
  assert.match(packager, /Packaged MODRIK release identity is invalid/);
  assert.match(remote, /Switching CloudLinux\/LiteSpeed startup to direct Next standalone server/);
