...
require_web_release() {
  local body="$1"
  [[ "$body" == *'data-testid="modrik-web-release-badge"'* ]] \
...
if ! require_web_release "$web_body"; then
  echo 'MODRIK_DEPLOY_WEB_RELEASE_MISMATCH' >&2
  exit 1
fi

if [[ "$web_body" != *'data-testid="modrik-landing-page"'* ]] \
  || [[ "$web_body" != *'data-testid="modrik-student-portal-cta"'* ]] \
  || [[ "$web_body" != *'href="/student"'* ]]; then
  echo 'MODRIK_DEPLOY_LANDING_ROUTE_MISMATCH' >&2
  exit 1
fi
...
if ! require_web_release "$student_body"; then
  echo 'MODRIK_DEPLOY_STUDENT_RELEASE_MISMATCH' >&2
  exit 1
fi

if [[ "$student_body" != *'data-testid="modrik-student-portal-route"'* ]] \
  || [[ "$student_body" != *'class="auth-shell"'* ]]; then
  echo 'MODRIK_DEPLOY_STUDENT_ROUTE_MISMATCH' >&2
  exit 1
fi
...
echo "MODRIK_DEMO_RELEASE_SMOKE_OK release=$SHORT_SHA landing=PASS student=PASS admin=PASS"
