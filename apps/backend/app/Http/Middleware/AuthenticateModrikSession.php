...
final class AuthenticateModrikSession
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        $session = $this->auth->authenticateProductionToken($request->bearerToken());
        if ($session !== null) {
            $request->setUserResolver(static fn (): User => $session['user']);
            $request->attributes->set('auth_session_id', $session['session_id']);
