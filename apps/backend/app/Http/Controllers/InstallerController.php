...
final class InstallerController extends Controller
{
    ...
    public function submit(Request $request, InstallerService $installer, InstallerRequirements $requirements): RedirectResponse
    {
        ...
        try {
            ...
            return redirect()->route('installer.finish')->with('installation_completed', true);
        } catch (Throwable) {
    ...
    {
        if (! $state->installed() || $request->session()->pull('installation_completed') !== true) {
